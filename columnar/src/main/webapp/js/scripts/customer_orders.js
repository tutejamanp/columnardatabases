			...
			order.ordereditemlist.forEach(function(product){
				...
				console.log(product);
					...
					$.get(api , function(data, status){
					    ...
					    Template = Template +"<figcaption class='media-body'><h6 class='title text-truncate'>"+productInfo.productname+" </h6>" + "<h6 class='title text-truncate'> Manufacturer : "+productInfo.manufacturer+" </h6>" ;
...
function setrecieved(orderid, productid) {
	...
	$.ajax({
        ...
        success:function(result){
        	...
        	window.location.href = "leave_review.jsp";
        },
        error: function(data){
            alert("fail");
        }
